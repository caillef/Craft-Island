...
--The Up arrow button on the player list UI
local propUp = script:GetCustomProperty("Up"):WaitForObject()

--The Down arrow button on the player list UI
local propDown = script:GetCustomProperty("Down"):WaitForObject()

--The parent object for the player list
local propPlayerList = script:GetCustomProperty("PlayerList"):WaitForObject()

--The Online button
local propOnline = script:GetCustomProperty("Online"):WaitForObject()
...
--The button for closing the player info panel
local closeButton = propPlayerInfo:FindDescendantByName("Close")
...
--The health bar and text in the player info panel
local infoHealthBar = propPlayerInfo:GetCustomProperty("Health"):GetObject()
local infoHealthText = propPlayerInfo:GetCustomProperty("HealthText"):GetObject()

--The key binding for opening and closing the player list
local propKeyBinding = script:GetCustomProperty("KeyBinding")

--The initial string for the health text
local initialHealthString = infoHealthText.text

--The replacement strings to look for
local healthReplacementLeft = "{current}"
local healthReplacementRight = "{total}"
...
function Tick(deltaTime)
	...
	if isMoving then
		...
		if targetDestination == OPEN_Y then
			newPosition = CoreMath.Lerp(CLOSED_Y, OPEN_Y, (TOGGLE_TIME - timeRemaining) / TOGGLE_TIME)
		else
			newPosition = CoreMath.Lerp(OPEN_Y, CLOSED_Y, (TOGGLE_TIME - timeRemaining) / TOGGLE_TIME)
		end
		
		--Update the position
		propPlayerList.y = newPosition
...
local function Expand()
	...
	--Switch which button is visible on the UI
	propUp.visibility = Visibility.FORCE_OFF
	propDown.visibility = Visibility.INHERIT
end
...
local function Collapse()
	...
	--Switch which button is visible on the UI
	propUp.visibility = Visibility.INHERIT
	propDown.visibility = Visibility.FORCE_OFF
...
function DisplayPlayerPanel(button, player)
	...
	local newHealthText = string.format("%d/%d", player.hitPoints, player.maxHitPoints)
	infoHealthText.text = newHealthText 
	infoHealthBar.progress = player.hitPoints / player.maxHitPoints
end
...
local function OnPlayerJoined(player)
	...
	displaysInUse[player]:FindChildByName("Pic"):SetImage(player)
		
	--If the player is the local player, then append "(You)" onto the end of the name	
	if player == localPlayer then
		displaysInUse[player]:FindChildByName("Name").text = player.name .. " (You)"
	else
...
local function Init()
	...
	--Connect some events
	propUp.clickedEvent:Connect(Expand)
	propDown.clickedEvent:Connect(Collapse)
	propOnline.clickedEvent:Connect(SwitchDisplay, DISPLAYS.ONLINE)
	Game.playerJoinedEvent:Connect(OnPlayerJoined)
	Game.playerLeftEvent:Connect(OnPlayerLeft)
	Events.Connect("Open", OpenPlayerInfo)
	closeButton.clickedEvent:Connect(ClosePlayerInfo)
	Events.Connect("HidePlayers", Hide)
	Events.Connect("ShowPlayers", Show)
	localPlayer.bindingPressedEvent:Connect(OnBindingPressed)
	
	--Set the health bar to full
	infoHealthBar.progress = 1
end
